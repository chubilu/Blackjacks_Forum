		...
		<?php $_topic_review_row_count = (isset($this->_tpldata['topic_review_row'])) ? sizeof($this->_tpldata['topic_review_row']) : 0;if ($_topic_review_row_count) {for ($_topic_review_row_i = 0; $_topic_review_row_i < $_topic_review_row_count; ++$_topic_review_row_i){$_topic_review_row_val = &$this->_tpldata['topic_review_row'][$_topic_review_row_i]; if (!($_topic_review_row_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="row1"><?php } else { ?><tr class="row2"><?php } if ($_topic_review_row_val['S_IGNORE_POST']) {  ?>
				...
				<td width="100%">
					...
					<tr>
						...
						<td valign="top" nowrap="nowrap">&nbsp;<?php if ($_topic_review_row_val['POSTER_QUOTE'] && $_topic_review_row_val['DECODED_MESSAGE']) {  ?><a class="cite" href="#" onclick="addquote(<?php echo $_topic_review_row_val['POST_ID']; ?>,'<?php echo $_topic_review_row_val['POSTER_QUOTE']; ?>', '<?php echo ((isset($this->_rootref['LA_WROTE'])) ? $this->_rootref['LA_WROTE'] : ((isset($this->_rootref['L_WROTE'])) ? addslashes($this->_rootref['L_WROTE']) : ((isset($user->lang['WROTE'])) ? addslashes($user->lang['WROTE']) : '{ WROTE }'))); ?>'); return false;"><span><?php echo (isset($this->_rootref['QUOTE_IMG'])) ? $this->_rootref['QUOTE_IMG'] : ''; ?></span></a><?php } ?></td>
					</tr>
					</table>
				</td>
			</tr>
					...
					<tr>
						<td>
							...
							<tr valign="middle">
								...
								<td class="gensmall2" nowrap="nowrap"><b><?php echo ((isset($this->_rootref['L_POSTED'])) ? $this->_rootref['L_POSTED'] : ((isset($user->lang['POSTED'])) ? $user->lang['POSTED'] : '{ POSTED }')); ?>:</b> <?php echo $_topic_review_row_val['POST_DATE']; ?></td>
							</tr>
							</table>
						</td>
					</tr>
