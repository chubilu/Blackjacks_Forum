<?php if (!defined('IN_PHPBB')) exit; ?><form id="users_mchat" method="post" action="<?php echo (isset($this->_rootref['U_ACTION'])) ? $this->_rootref['U_ACTION'] : ''; ?>">
	...
	<dl>
		<dt><label for="mchat_input_type"><?php echo ((isset($this->_rootref['L_CHAT_AREA'])) ? $this->_rootref['L_CHAT_AREA'] : ((isset($user->lang['CHAT_AREA'])) ? $user->lang['CHAT_AREA'] : '{ CHAT_AREA }')); ?>:</label></dt>
		<dd>
			<label><input type="radio" name="user_mchat_input_area" value="1"<?php if ($this->_rootref['INPUT_AREA']) {  ?> id="mchat_input_type" checked="checked"<?php } ?> /> <?php echo ((isset($this->_rootref['L_INPUT_AREA'])) ? $this->_rootref['L_INPUT_AREA'] : ((isset($user->lang['INPUT_AREA'])) ? $user->lang['INPUT_AREA'] : '{ INPUT_AREA }')); ?></label> 
			<label><input type="radio" name="user_mchat_input_area" value="0"<?php if (! $this->_rootref['INPUT_AREA']) {  ?> id=""mchat_input_type" checked="checked"<?php } ?> /> <?php echo ((isset($this->_rootref['L_TEXT_AREA'])) ? $this->_rootref['L_TEXT_AREA'] : ((isset($user->lang['TEXT_AREA'])) ? $user->lang['TEXT_AREA'] : '{ TEXT_AREA }')); ?></label>
		</dd>
	</dl>	
	</fieldset>
