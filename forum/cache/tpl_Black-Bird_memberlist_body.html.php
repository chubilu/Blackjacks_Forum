<?php if (!defined('IN_PHPBB')) exit; if ($this->_rootref['S_IN_SEARCH_POPUP']) {  $this->_tpl_include('simple_header.html'); } else { $this->_tpl_include('overall_header.html'); } if ($this->_rootref['S_SEARCH_USER']) {  $this->_tpl_include('memberlist_search.html'); } if ($this->_rootref['S_SHOW_GROUP']) {  $this->_tpl_include('memberlist_group.html'); } if (! $this->_rootref['S_SHOW_GROUP']) {  ?>
	...
	<form method="post" name="charsearch" action="<?php echo (isset($this->_rootref['S_MODE_ACTION'])) ? $this->_rootref['S_MODE_ACTION'] : ''; ?>">
		...
		<tr>
			<td align="<?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>"><span class="genmeds"><?php echo ((isset($this->_rootref['L_USERNAME_BEGINS_WITH'])) ? $this->_rootref['L_USERNAME_BEGINS_WITH'] : ((isset($user->lang['USERNAME_BEGINS_WITH'])) ? $user->lang['USERNAME_BEGINS_WITH'] : '{ USERNAME_BEGINS_WITH }')); ?>: </span><select name="first_char" onchange="this.form.submit();"><?php echo (isset($this->_rootref['S_CHAR_OPTIONS'])) ? $this->_rootref['S_CHAR_OPTIONS'] : ''; ?></select>&nbsp;<input type="submit" name="char" value="<?php echo ((isset($this->_rootref['L_DISPLAY'])) ? $this->_rootref['L_DISPLAY'] : ((isset($user->lang['DISPLAY'])) ? $user->lang['DISPLAY'] : '{ DISPLAY }')); ?>" class="btnlite" /></td>
	<?php if ($this->_rootref['U_FIND_MEMBER'] && ! $this->_rootref['S_SEARCH_USER']) {  ?>

			<td class="genmeds" align="<?php echo (isset($this->_rootref['S_CONTENT_FLOW_END'])) ? $this->_rootref['S_CONTENT_FLOW_END'] : ''; ?>"><a href="<?php echo (isset($this->_rootref['U_FIND_MEMBER'])) ? $this->_rootref['U_FIND_MEMBER'] : ''; ?>"><?php echo ((isset($this->_rootref['L_FIND_USERNAME'])) ? $this->_rootref['L_FIND_USERNAME'] : ((isset($user->lang['FIND_USERNAME'])) ? $user->lang['FIND_USERNAME'] : '{ FIND_USERNAME }')); ?></a></td>
	<?php } else if ($this->_rootref['S_SEARCH_USER'] && $this->_rootref['U_HIDE_FIND_MEMBER'] && ! $this->_rootref['S_IN_SEARCH_POPUP']) {  ?>

			<td class="genmeds" align="<?php echo (isset($this->_rootref['S_CONTENT_FLOW_END'])) ? $this->_rootref['S_CONTENT_FLOW_END'] : ''; ?>"><a href="<?php echo (isset($this->_rootref['U_HIDE_FIND_MEMBER'])) ? $this->_rootref['U_HIDE_FIND_MEMBER'] : ''; ?>"><?php echo ((isset($this->_rootref['L_HIDE_MEMBER_SEARCH'])) ? $this->_rootref['L_HIDE_MEMBER_SEARCH'] : ((isset($user->lang['HIDE_MEMBER_SEARCH'])) ? $user->lang['HIDE_MEMBER_SEARCH'] : '{ HIDE_MEMBER_SEARCH }')); ?></a></td>
	<?php } ?>
...
<?php $_memberrow_count = (isset($this->_tpldata['memberrow'])) ? sizeof($this->_tpldata['memberrow']) : 0;if ($_memberrow_count) {for ($_memberrow_i = 0; $_memberrow_i < $_memberrow_count; ++$_memberrow_i){$_memberrow_val = &$this->_tpldata['memberrow'][$_memberrow_i]; if ($this->_rootref['S_SHOW_GROUP']) {  if ($_memberrow_val['S_FIRST_ROW'] && $_memberrow_val['S_GROUP_LEADER']) {  ?>
			...
			<tr class="row3">
				<td colspan="8" class="row1"><b class="gensmall"><?php echo ((isset($this->_rootref['L_GROUP_LEADER'])) ? $this->_rootref['L_GROUP_LEADER'] : ((isset($user->lang['GROUP_LEADER'])) ? $user->lang['GROUP_LEADER'] : '{ GROUP_LEADER }')); ?></b></td>
			</tr>
		<?php } else if (! $_memberrow_val['S_GROUP_LEADER'] && ! $this->_tpldata['DEFINE']['.']['S_MEMBER_HEADER']) {  ?>

			<tr class="row3">
				<td colspan="8" class="row1"><b class="gensmall"><?php echo ((isset($this->_rootref['L_GROUP_MEMBERS'])) ? $this->_rootref['L_GROUP_MEMBERS'] : ((isset($user->lang['GROUP_MEMBERS'])) ? $user->lang['GROUP_MEMBERS'] : '{ GROUP_MEMBERS }')); ?></b></td>
			</tr>
				<?php $this->_tpldata['DEFINE']['.']['S_MEMBER_HEADER'] = 1; } } ?>


	<tr>
		<td class="gen<?php if (!($_memberrow_val['S_ROW_COUNT'] & 1)  ) {  ?> row2<?php } else { ?> row1<?php } ?>" align="center">&nbsp;<?php echo $_memberrow_val['ROW_NUMBER']; ?>&nbsp;</td>
		<td class="genmed<?php if (!($_memberrow_val['S_ROW_COUNT'] & 1)  ) {  ?> row2<?php } else { ?> row1<?php } ?>" align="<?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>"><?php echo $_memberrow_val['USERNAME_FULL']; if ($this->_rootref['S_SELECT_SINGLE']) {  ?> [&nbsp;<a href="#" onclick="insert_single('<?php echo $_memberrow_val['A_USERNAME']; ?>'); return false;"><?php echo ((isset($this->_rootref['L_SELECT'])) ? $this->_rootref['L_SELECT'] : ((isset($user->lang['SELECT'])) ? $user->lang['SELECT'] : '{ SELECT }')); ?></a>&nbsp;]<?php } ?></td>
		<td class="genmed<?php if (!($_memberrow_val['S_ROW_COUNT'] & 1)  ) {  ?> row2<?php } else { ?> row1<?php } ?>" align="center" nowrap="nowrap">&nbsp;<?php echo $_memberrow_val['JOINED']; ?>&nbsp;</td>
		<td class="gen<?php if (!($_memberrow_val['S_ROW_COUNT'] & 1)  ) {  ?> row2<?php } else { ?> row1<?php } ?>" align="center"><?php echo $_memberrow_val['POSTS']; ?></td>
		<td class="gen<?php if (!($_memberrow_val['S_ROW_COUNT'] & 1)  ) {  ?> row2<?php } else { ?> row1<?php } ?>" align="center"><?php if ($_memberrow_val['RANK_IMG']) {  echo $_memberrow_val['RANK_IMG']; } else { echo $_memberrow_val['RANK_TITLE']; } ?></td>
		<td class="gen<?php if (!($_memberrow_val['S_ROW_COUNT'] & 1)  ) {  ?> row2<?php } else { ?> row1<?php } ?>" align="center">&nbsp;<?php if ($_memberrow_val['U_PM']) {  ?><a class="mp"href="<?php echo $_memberrow_val['U_PM']; ?>"><span><?php echo (isset($this->_rootref['PM_IMG'])) ? $this->_rootref['PM_IMG'] : ''; ?></span></a><?php } ?></td>
		<td class="gen<?php if (!($_memberrow_val['S_ROW_COUNT'] & 1)  ) {  ?> row2<?php } else { ?> row1<?php } ?>" align="center">&nbsp;<?php if ($_memberrow_val['U_EMAIL']) {  ?><a class="mail" href="<?php echo $_memberrow_val['U_EMAIL']; ?>"><span><?php echo (isset($this->_rootref['EMAIL_IMG'])) ? $this->_rootref['EMAIL_IMG'] : ''; ?></span></a><?php } ?></td>
		<td class="gen<?php if (!($_memberrow_val['S_ROW_COUNT'] & 1)  ) {  ?> row2<?php } else { ?> row1<?php } ?>" align="center">&nbsp;<?php if ($_memberrow_val['U_WWW']) {  ?><a class="www" href="<?php echo $_memberrow_val['U_WWW']; ?>"><span><?php echo (isset($this->_rootref['WWW_IMG'])) ? $this->_rootref['WWW_IMG'] : ''; ?></span></a><?php } ?></td>
		<?php if ($_memberrow_val['S_PROFILE_FIELD1']) {  ?><!-- Use a construct like this to include admin defined profile fields. Replace FIELD1 with the name of your field. -->
			<td class="gen<?php if (!($_memberrow_val['S_ROW_COUNT'] & 1)  ) {  ?> row2<?php } else { ?> row1<?php } ?>" align="center">&nbsp;<?php echo $_memberrow_val['PROFILE_FIELD1_VALUE']; ?></td>
		<?php } if ($this->_rootref['S_IN_SEARCH_POPUP'] && ! $this->_rootref['S_SELECT_SINGLE']) {  ?><td align="center" class="<?php if (!($_memberrow_val['S_ROW_COUNT'] & 1)  ) {  ?> row2<?php } else { ?> row1<?php } ?>"><input type="checkbox" class="radio" name="user" value="<?php echo $_memberrow_val['USERNAME']; ?>" /></td><?php } ?>
