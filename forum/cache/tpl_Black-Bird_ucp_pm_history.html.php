...
<tr>
	...
	<?php $_history_row_count = (isset($this->_tpldata['history_row'])) ? sizeof($this->_tpldata['history_row']) : 0;if ($_history_row_count) {for ($_history_row_i = 0; $_history_row_i < $_history_row_count; ++$_history_row_i){$_history_row_val = &$this->_tpldata['history_row'][$_history_row_i]; ?>
			...
			<tr>
			<td class="<?php if (!($_history_row_val['S_ROW_COUNT'] & 1)  ) {  ?>row1<?php } else { ?>row2<?php } ?>" rowspan="2" align="<?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>" valign="top"><a name="<?php echo $_history_row_val['MSG_ID']; ?>"></a>
			...
			<td class="<?php if (!($_history_row_val['S_ROW_COUNT'] & 1)  ) {  ?>row1<?php } else { ?>row2<?php } ?>" width="100%"<?php if ($_history_row_val['S_CURRENT_MSG']) {  ?> style="background: #1B1B1B"<?php } ?>>
		...
		<tr>
			<td class="<?php if (!($_history_row_val['S_ROW_COUNT'] & 1)  ) {  ?>row1<?php } else { ?>row2<?php } ?>" valign="top">
		...
		<tr>
			<td class="gensmall<?php if (!($_history_row_val['S_ROW_COUNT'] & 1)  ) {  ?> row1<?php } else { ?> row2<?php } ?>"><a href="<?php echo $_history_row_val['U_VIEW_MESSAGE']; ?>"><?php echo ((isset($this->_rootref['L_VIEW_PM'])) ? $this->_rootref['L_VIEW_PM'] : ((isset($user->lang['VIEW_PM'])) ? $user->lang['VIEW_PM'] : '{ VIEW_PM }')); ?></a></td>
			<td class="<?php if (!($_history_row_val['S_ROW_COUNT'] & 1)  ) {  ?>row1<?php } else { ?>row2<?php } ?>"><div class="gensmall" style="float: <?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>;">&nbsp;<?php if ($this->_rootref['U_MESSAGE_AUTHOR']) {  ?><a href="<?php echo (isset($this->_rootref['U_MESSAGE_AUTHOR'])) ? $this->_rootref['U_MESSAGE_AUTHOR'] : ''; ?>"><?php echo (isset($this->_rootref['PROFILE_IMG'])) ? $this->_rootref['PROFILE_IMG'] : ''; ?></a> <?php } if ($this->_rootref['U_EMAIL']) {  ?><a href="<?php echo (isset($this->_rootref['U_EMAIL'])) ? $this->_rootref['U_EMAIL'] : ''; ?>"><?php echo (isset($this->_rootref['EMAIL_IMG'])) ? $this->_rootref['EMAIL_IMG'] : ''; ?></a> <?php } ?>&nbsp;</div> 
